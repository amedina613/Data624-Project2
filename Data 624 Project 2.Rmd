...
There is one character column in the data set that is separated prior to kNN imputation using the Caret::preProcess function knnImpute. This method does not support categorical columns, but we can impute this column separately. kNN imputation is performed on the numeric columns.
...
preProcess_model <- preProcess(numeric_data, method = c("knnImpute"))
...
```
This plot does not look great, there are unrealistic pH values. The standardization of kNN imputation may lead to difficulty interpreting predictions directly in the original scale.
