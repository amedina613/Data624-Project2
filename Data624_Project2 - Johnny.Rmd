...
```{r}
colSums(is.na(studentdata_mice)) # Counts of missing values by column
```
...
correlation_matrix <- ggpairs(
  ...
  upper = list(continuous = wrap("cor", size = 3)),  # Add correlation values
...
plot_flow <- ggplot(studentdata_mice, aes(x = Mnf.Flow, y = PH)) +
  geom_point(color = "green", alpha = 0.6) +  # Scatterplot points
  geom_smooth(method = "lm", color = "red", se = TRUE) +
  labs(
